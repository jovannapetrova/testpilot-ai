...
class CoverageAgent:
    ...
    def run(self, project_dir: Path, execute: bool = True) -> CoverageResult:
        if not execute:
            return self._estimated(
                project_dir,
                "Test execution is disabled by configuration.",
        ...
        return self._estimated(
            project_dir,
            "No supported test runner or test directory was detected.",
            evidence_state="no_supported_runner",
        )
    ...
    def _run_python_coverage(self, project_dir: Path) -> CoverageResult:
        ...
        if not self._has_test_files(project_dir, {".py"}):
            return self._estimated(
                project_dir,
                "Python files detected but no pytest-compatible test files were found.",
                evidence_state="tests_not_detected",
            )
        ...
        try:
            ...
            return self._estimated(
                project_dir,
                f"Python coverage execution failed: {exc}",
    ...
    def _run_node_coverage(self, project_dir: Path) -> CoverageResult:
        ...
        try:
            ...
            return self._estimated(
                ...
                evidence_state="manifest_unreadable",
            )
        ...
        if "test" not in scripts:
            return self._estimated(
                ...
                evidence_state="test_script_missing",
            )
        ...
        try:
            ...
            result.output = (proc.stdout + "\n" + proc.stderr).strip()
            result.passed, result.failed = self._parse_test_counts(result.output)
            self._set_measured_coverage(result, self._parse_percent_from_text(result.output))
            self._coverage_reasons(result)
            return result
        except Exception as exc:
            return self._estimated(
                project_dir,
                f"Node/Jest coverage execution failed: {exc}",
                evidence_state="execution_failed",
            )
    ...
    def _run_command_coverage(
        ...
        try:
            ...
            if not result.available:
                result.reason = "Coverage percentage was not available in command output; inspect generated JaCoCo reports."
                result.evidence_state = "tests_detected_no_coverage_report"
            self._coverage_reasons(result)
            return result
        except Exception as exc:
            return self._estimated(
                project_dir,
                f"{tool} coverage execution failed: {exc}",
                evidence_state="execution_failed",
            )

    def _estimated(self, project_dir: Path, reason: str, evidence_state: str = "estimated_unavailable") -> CoverageResult:
        ...
        result = CoverageResult(
            ...
            estimated_coverage_percent=estimate,
            display_label="Not measured",
            evidence_state=evidence_state,
            output=f"Coverage not measured. {reason}",
            reason=reason,
    ...
    def _apply_coverage_json(self, result: CoverageResult, raw: str) -> None:
        try:
            ...
            self._set_measured_coverage(result, self._parse_percent_from_text(raw))
            if not result.available and not result.reason:
                result.reason = "Coverage JSON report was not produced or could not be parsed."
                result.evidence_state = "tests_detected_no_coverage_report"
            return

        totals = data.get("totals") or {}
        if "percent_covered" not in totals:
            self._set_measured_coverage(result, None)
            if not result.reason:
                result.reason = "Coverage JSON report did not include a measured percent_covered total."
                result.evidence_state = "tests_detected_no_coverage_report"
            return

        self._set_measured_coverage(result, totals.get("percent_covered"))
    ...
    def _set_measured_coverage(self, result: CoverageResult, value: float | str | None) -> None:
        if value is None:
            result.coverage_percent = 0.0
            result.measured = False
            result.available = False
            result.display_label = "Not measured"
            if not result.evidence_state or result.evidence_state == "unavailable":
                result.evidence_state = "tests_detected_no_coverage_report"
            return
        ...
        result.display_label = f"{percent:g}%"
        result.evidence_state = "measured"

    def _coverage_reasons(self, result: CoverageResult) -> None:
        if not result.available or not result.measured:
            if result.reason:
                result.low_coverage_reasons.append(result.reason)
            else:
                result.low_coverage_reasons.append("Coverage was not measured for this analysis.")
            if result.estimated and result.estimated_coverage_percent is not None:
                result.low_coverage_reasons.append(
                    f"Static test/source-file ratio suggests roughly {result.estimated_coverage_percent:g}% evidence, but this is not measured coverage."
                )
            return
        ...
        if result.coverage_percent == 0:
            result.low_coverage_reasons.append("Measured coverage is 0%.")
    ...
    def _parse_percent_from_text(self, text: str) -> float | None:
